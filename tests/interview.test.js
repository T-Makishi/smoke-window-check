...
import {createPresentedResult,INTERVIEW_MAX_QUESTIONS,interviewQuestionsFor,interviewRows,photoGuidanceFor,pruneInterviewAnswers,presentedResultRows} from '../js/interview-config.js';
...
test('顧客提示結果と業者メールへ同じスナップショットを使う',()=>{
  ...
  assert.ok(interviewRows(draft.diagnosis).some(([,answer])=>answer==='ほつれている'));
...
test('症状選択後は一問ずつ回答する画面を表示する',()=>{
  ...
  assert.match(html,/質問は最大5問です/);
});

test('複数の症状を選んでも質問は最大5問に制限する',()=>{
  const draft=createDraft();
  draft.diagnosis.symptoms=['wontOpen','wontClose','spins','heavy','noise','wire','detached','stops','leak','unknown','other'];
  const questions=interviewQuestionsFor(draft.diagnosis);
  assert.equal(questions.length,INTERVIEW_MAX_QUESTIONS);
  assert.equal(questions[0].id,'falling_risk');
  assert.ok(questions.some(question=>question.id==='wire_state'));
  assert.ok(questions.some(question=>question.id==='detached_state'));
});
