...
async function tenantAction(event){
  ...
  if(button.dataset.action==='reset-email-limit'){showAuthenticationResetDialog(tenant,'email');return}
  if(button.dataset.action==='reset-passcode-lock'){showAuthenticationResetDialog(tenant,'passcode');return}
...
function showAuthenticationResetDialog(tenant,type){
  const email=type==='email',title=email?'確認メール送信制限を解除':'パスコード入力ロックを解除';
  const explanation=email?'この業者の確認メール送信回数を0に戻します。発行済みの未使用リンクはすべて無効になります。':'この業者のパスコード入力失敗回数と15分ロックだけを解除します。登録済みパスコードは変更しません。';
  showDialog(title,`<p><strong>${escapeHtml(tenant.companyName)}</strong>だけが対象です。</p><p>${explanation}</p><p class="hint">他社のログイン、問診データ、利用期間には影響しません。実行日時と運営者メールを監査記録に保存します。</p><div class="field"><label for="authResetCompanyName">確認のため会社名を入力</label><input id="authResetCompanyName" autocomplete="off" placeholder="${escapeHtml(tenant.companyName)}"></div><p class="error" id="dialogError" hidden></p>`,title,async()=>{
    const confirmation=document.querySelector('#authResetCompanyName').value.trim(),error=document.querySelector('#dialogError'),button=document.querySelector('#dialogSave');
    if(confirmation!==tenant.companyName){showError(error,'会社名が一致しません。');return}
    button.disabled=true;
    try{const result=await api('/api/service/tenants',{method:'PATCH',body:{id:tenant.id,action:email?'reset_login_email_limit':'reset_passcode_lock',confirmCompanyName:confirmation}});closeDialog();await loadTenants();toast(email?`確認メール送信制限を解除し、未使用リンク${result.invalidatedUnusedLinks}件を無効化しました。`:'パスコード入力ロックを解除しました。')}
