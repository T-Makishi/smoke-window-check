...
import {customerFields,renderCustomerGuide,renderDetail,renderHistory,renderHome,renderSettings,renderStep} from './ui.js';
...
const tenantId=readTenantId(location.href),tenantParamPresent=isTenantUrl(location.href),demoMode=new URL(location.href).searchParams.get('demo')==='1',embedMode=new URL(location.href).searchParams.get('embed')==='1';
const platform={tenantId,tenant:null,vendorAuthenticated:false,passcode:null,userEmail:'',initializing:true,blocked:false,demo:demoMode,embed:embedMode};
...
async function initialize(){
  ...
  if(platform.demo){settings=structuredClone(DEFAULT_SETTINGS);settings.company.name='排煙窓事前見積デモ';settings.company.tradeName='操作体験用';settings.company.phone='';settings.company.email='demo@example.invalid';settings.app.name='排煙窓事前見積 操作体験';document.body.classList.add('is-demo');if(platform.embed)document.body.classList.add('is-embedded');document.querySelector('#settingsButton').hidden=true;platform.initializing=false;applySettings();renderHomeScreen();return}
...
async function handleClick(e){
  ...
  if(action){
    ...
    if(action==='print-guide')printCustomerGuide()
...
function sendEmail(c){if(platform.demo){showModal('操作体験はここまでです','<p>本番では、ここから業者の受付メール宛てに問診内容を送信します。</p><p>この操作体験では入力内容を保存・送信していません。</p>');return}try{location.href=buildMailto(c,settings)}catch(e){toast(e.message)}}
...
async function saveAndShareCustomerLink(){
  ...
  const title='排煙窓事前チェック',text=`${settings.company.name}の排煙窓事前チェックです。写真と簡単な質問で症状をお知らせください。`;
...
async function renderQrShareCard(qr,canvas,currentSettings){
  ...
  context.fillStyle='#0d2b45';context.font='700 43px -apple-system, BlinkMacSystemFont, "Noto Sans JP", sans-serif';context.fillText('写真と簡単な質問で修理費用の目安を確認',width/2,1430);
...
async function shareQrImage(canvas){try{const blob=await qrCanvasBlob(canvas),file=new File([blob],'排煙窓事前チェック_LINE用QR画像.png',{type:'image/png'});if(!navigator.canShare?.({files:[file]}))throw new Error();await navigator.share({title:'排煙窓事前チェック',text:`${settings.company.name}の排煙窓事前チェックです。画像のQRコードから開いてください。`,files:[file]})}catch(e){if(e?.name!=='AbortError')toast('この端末では画像共有を利用できません。「PNGで保存」をご利用ください。')}}
...
function enhanceShareControls(){
  ...
  const heading=card?.querySelector('h2');if(heading)heading.textContent='お客様へ問診ページを案内する';
  const description=card?.querySelector('h2 + p');if(description)description.textContent='会社情報と受付メールを確認したら、下の方法からお客様へ専用問診ページを送ります。';
  const steps=document.createElement('ol');steps.className='share-steps';steps.innerHTML='<li><strong>1</strong><span>会社情報・料金・受付先を確認</span></li><li><strong>2</strong><span>LINE・メール・QRで依頼者へ案内</span></li><li><strong>3</strong><span>事前問診を受付メールで確認</span></li>';
...
function requestSettingsAccess(){if(platform.tenantId){if(platform.vendorAuthenticated){appState.screen='settings';render();return}const emailHint=platform.tenant?.vendorEmailHint?`<p class="login-email-hint"><strong>登録済みメール：</strong>${escapeHtml(platform.tenant.vendorEmailHint)}</p>`:'';showModal('業者ログイン（招待済みの方）',`<p><strong>この画面ではメールアドレスを新規登録できません。</strong></p><p>次の画面で、運営者が登録したメールアドレスを入力すると確認コードが届きます。</p>${emailHint}<p class="hint">表示されたメールに心当たりがない場合は、サービス運営者へ登録メールの変更をご依頼ください。確認コードが届かない場合は、認証画面から再送できます。</p>`,()=>{location.href=vendorLoginUrl(platform.tenantId,location.href)});const save=document.querySelector('#modalSave');save.textContent='登録済みメールで確認コードを受け取る';return}showModal('業者設定を開く','<p>この画面は排煙窓会社の担当者専用です。</p><div class="field"><label for="settingsPasscode">パスコード</label><input id="settingsPasscode" type="password" inputmode="numeric" maxlength="8" autocomplete="current-password"><p class="error" id="passcodeError" hidden></p></div>',verifySettingsPasscode);const save=document.querySelector('#modalSave');save.textContent='確認して開く';document.querySelector('#settingsPasscode').focus()}
...
function enhanceGuideVisuals(){const steps=document.querySelector('.guide-steps');if(!steps||document.querySelector('.guide-visuals'))return;const visuals=document.createElement('section');visuals.className='guide-visuals';visuals.setAttribute('aria-label','スマートフォン画面で見る操作手順');visuals.innerHTML=`<h2>スマートフォン画面で見る操作手順</h2><p class="guide-visuals__lead">赤い枠の場所を順番に押してください。</p><div class="guide-phone-grid"><figure><div class="guide-phone"><div class="guide-phone__bar">${escapeHtml(settings.company.name)}</div><div class="guide-phone__screen guide-phone__home"><small>訪問前の排煙窓</small><strong>事前チェック</strong><span class="guide-tap-target">事前チェックを始める</span></div></div><figcaption><b>1</b><span>緑色の「事前チェックを始める」を押します。</span></figcaption></figure><figure><div class="guide-phone"><div class="guide-phone__bar">症状を選ぶ</div><div class="guide-phone__screen"><small>当てはまる症状をすべて選択</small><span class="guide-choice guide-tap-target">動きが重い　✓</span><span class="guide-choice">異音がする</span><span class="guide-next">次へ進む ›</span></div></div><figcaption><b>2</b><span>症状は複数選べます。最後に「次へ進む」を押します。</span></figcaption></figure><figure><div class="guide-phone"><div class="guide-phone__bar">写真・動画</div><div class="guide-phone__screen"><small>全体・操作部分・故障箇所</small><span class="guide-upload guide-tap-target">写真・動画を選ぶ</span><span class="guide-next">次へ進む ›</span></div></div><figcaption><b>3</b><span>排煙窓の写真を選びます。分からなければ次へ進めます。</span></figcaption></figure><figure><div class="guide-phone"><div class="guide-phone__bar">保存完了</div><div class="guide-phone__screen"><strong>まだ送信されていません</strong><small>メール画面で写真を添付してください</small><span class="guide-tap-target">問診内容をメールで送る</span></div></div><figcaption><b>4</b><span>メール画面を開き、写真を添付して送信します。</span></figcaption></figure></div>`;steps.before(visuals)}
function printCustomerGuide(){if(appState.screen!=='guide'){appState.screen='guide';render()}const previousTitle=document.title;document.title=`${settings.company.name}_排煙窓事前チェック操作ガイド`;document.body.classList.add('printing-guide');const restore=()=>{document.title=previousTitle;document.body.classList.remove('printing-guide')};requestAnimationFrame(()=>{try{window.print();restore();setTimeout(()=>toast('印刷画面で「PDFとして保存」を選ぶとPDFファイルにできます。'),100)}catch{restore();toast('印刷画面を開けませんでした。ブラウザのメニューから「印刷」または「プリント」を選択してください。')}})}
