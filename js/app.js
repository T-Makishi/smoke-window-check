...
function enhanceSettings(){const form=document.querySelector('#settingsForm');const first=form?.querySelector('.card');if(!form||!first)return;const logo=document.createElement('div');logo.className='field';logo.innerHTML='<label for="companyLogo">ロゴ画像</label><input id="companyLogo" type="file" accept="image/png,image/jpeg,image/webp"><p class="hint">PNG・JPEG・WebP形式、1MB以下。共有用に64×64pxへ自動圧縮します。</p><div class="logo-setting-status tool-row" id="companyLogoStatus"></div>';first.append(logo);updateLogoStatus();const card=document.createElement('div');card.className='card stack';const title=document.createElement('h2');title.textContent='詳細料金・訪問条件';card.append(title);const entries=[['キャンセル時の説明','inspection.cancellation','text'],['不在時の説明','inspection.absence','text'],['複数症状の追加確認費','pricing.additionalSymptomFee','number'],['ワイヤー交換 最低額','pricing.symptoms.wire.0','number'],['ワイヤー交換 最高額','pricing.symptoms.wire.1','number'],['ハンドル交換 最低額','pricing.symptoms.spins.0','number'],['ハンドル交換 最高額','pricing.symptoms.spins.1','number'],['オペレーター交換 最低額','pricing.symptoms.wontOpen.0','number'],['オペレーター交換 最高額','pricing.symptoms.wontOpen.1','number'],['調整作業 最低額','pricing.symptoms.heavy.0','number'],['調整作業 最高額','pricing.symptoms.heavy.1','number'],['その他作業 最低額','pricing.symptoms.other.0','number'],['その他作業 最高額','pricing.symptoms.other.1','number']];for(const [label,name,type] of entries){const wrap=document.createElement('div');wrap.className='field';const lab=document.createElement('label');lab.htmlFor=name;lab.textContent=label;const input=document.createElement('input');input.id=input.name=name;input.type=type;input.value=name.split('.').reduce((o,k)=>o?.[k],settings)??'';wrap.append(lab,input);card.append(wrap)}form.insertBefore(card,form.lastElementChild);if(!platform.tenantId){const security=document.createElement('div');security.className='card stack';security.innerHTML='<h2>業者設定のパスコード</h2><p class="hint">次回から設定画面を開くときに使用します。4〜8桁の数字を設定してください。空欄の場合は変更しません。</p><div class="field"><label for="newSettingsPasscode">新しいパスコード</label><input id="newSettingsPasscode" type="password" inputmode="numeric" maxlength="8" autocomplete="new-password"></div><div class="field"><label for="confirmSettingsPasscode">新しいパスコード（確認）</label><input id="confirmSettingsPasscode" type="password" inputmode="numeric" maxlength="8" autocomplete="new-password"></div>';form.insertBefore(security,form.lastElementChild)}form.addEventListener('input',updateCustomerUrlPreview);form.addEventListener('change',updateCustomerUrlPreview);updateCustomerUrlPreview()}
function enhanceShareControls(){const output=document.querySelector('#customerShareUrl'),field=output?.closest('.field'),message=document.querySelector('#customerShareUrlMessage'),original=document.querySelector('[data-action="save-and-copy-customer-url"]');if(!output||!field||!message||!original)return;output.type='hidden';field.querySelector('label')?.remove();const ready=document.createElement('div'),title=document.createElement('strong');ready.className='share-ready';title.textContent='お客様への案内を準備できます';ready.append(title,message);field.prepend(ready);original.dataset.action='save-and-copy-email-link';original.textContent='設定を保存してメール用リンクをコピー';const qrButton=document.createElement('button');qrButton.type='button';qrButton.className='button line-share-button';qrButton.dataset.action='save-and-show-qr';qrButton.textContent='設定を保存してLINE用QR画像を作成';const details=document.createElement('details');details.innerHTML='<summary>実際のURLが必要な場合</summary><p class="hint">システム登録などでURLそのものが必要な場合だけ使用してください。</p><button class="button ghost small" data-action="save-and-copy-customer-url" type="button">実際のURLをコピー</button>';original.after(qrButton,details)}
function updateCustomerUrlPreview(){const form=document.querySelector('#settingsForm'),output=document.querySelector('#customerShareUrl'),message=document.querySelector('#customerShareUrlMessage');if(!form||!output)return;try{output.value=platform.tenantId?buildTenantCustomerUrl(platform.tenantId,location.href):buildCustomerUrl(settingsFromForm(form,settings),location.href);output.removeAttribute('aria-invalid');if(message)message.textContent=platform.tenantId?'短いお客様用URLをメールまたはLINEで案内できます。':'メールには短い表示リンク、LINEにはQR案内画像を使用します。長いURLは通常表示しません。'}catch(e){output.value='';output.setAttribute('aria-invalid','true');if(message)message.textContent=e.message}}
