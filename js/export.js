...
export function caseText(c,settings){const d=c.diagnosis,cu=c.customer,result=c.presentedResult||createPresentedResult(c,c.estimate,settings,c.updatedAt||c.createdAt||new Date().toISOString());return [
  ...
  `症状：${symptomLabels(d).join('、')||SYMPTOMS[d.symptom]?.[0]||d.symptom}`,'',
  '【選択式問診の回答】',...interviewRows(d).map(([question,answer])=>`${question}：${answer}`),'',
  '【お客様に提示した事前チェック結果】',...presentedResultRows(result,{contractor:true}).map(([label,value])=>`${label}：${value}`),result.disclaimer&&`注意：${result.disclaimer}`,'',
  `設置高さ：${LABELS.height[d.heightType]||d.heightType}`,`対象数量：${d.quantity==='5plus'?'5か所以上':`${d.quantity}か所`}`,
