...
function scope(request,env){const email=requireServiceAdmin(request,env);return {email,subject:servicePasscodeSubject()}}

export async function onRequestGet({request,env}){
  try{const context=scope(request,env);return json({ok:true,user:{email:context.email},passcode:await passcodeStatus(env,{...context,request,cookieName:SERVICE_PASSCODE_COOKIE})})}
  catch(error){return handleError(error)}
}

export async function onRequestPost({request,env}){
  try{
    const context=scope(request,env),body=await readJson(request,4096),options={...context,request,cookieName:SERVICE_PASSCODE_COOKIE};
    let cookie;
...
export async function onRequestDelete({request,env}){
  try{scope(request,env);return json({ok:true},{headers:{'set-cookie':await destroyPasscodeSession(request,env,{cookieName:SERVICE_PASSCODE_COOKIE})}})}
  catch(error){return handleError(error)}
}
